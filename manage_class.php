...
<td style="border: 2px solid black; padding: 6px;"><center><a href="teacher_expel.php?Student_ID='.$row2['Student_ID'].'&Subject_ID='.$Subject_ID.'" type="button" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Remove</a></center></td>
</tr>
    ';
}
echo'</table>
...
<div style="width:650px; height:220px; border: 2px solid black" class="p-4">
<p class="text-xl">Add Students</p>
<br>
<form action="teacher_enroll.php" method="POST">
<label for="Name">Name</label><br>
        <select id="name" name="name" style="width:330px;" required>
        <option></option>';
$sqlnames="SELECT Student_ID, Full_Name FROM students where class='$class' and Course_ID='$Course_ID' and Student_ID not in (SELECT Student_ID from $Subject_ID)";
$resultnames= $conn->query($sqlnames);
while($rownames = $resultnames->fetch_assoc()){
  echo '<option value=' . $rownames['Student_ID'].'>'.$rownames['Full_Name'].'</option>';
}
        echo'
        </select>
        <br>
        <input type="hidden" name="Subject_ID" value="' . $Subject_ID . '"><br>
        <input type="submit" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" value="Add">
        </form>
        <br><br>
</div>
</center>
