...
$sql = "SELECT students.Student_ID, students.Full_Name, students.Class , courses.Name FROM students,courses WHERE courses.Course_ID=students.Course_ID";
$result = $conn->query($sql);    
$sql2="SELECT distinct Name FROM courses";
$result2 = $conn->query($sql2);
    ...
    if (isset($_SESSION["username"]) && isset($_SESSION["role"])) 
    {
        ...
        <p class="text-xl">Filter Students<p>
    ...
    <select style="width:300px;" type="text" id="filterCourse" name="filterCourse" oninput="filterTable()" />
    <option></option>';
while($row2 = $result2->fetch_assoc()){
    ...
    <select style="width:300px;" type="text" id="filterClass" name="filterClass" oninput="filterTable()" />
    <option></option>
...
<center>
        ...
        <th style="border: 4px solid black; padding: 8px;" class="text-2xl">View</th>
        </tr>
        ';
        while($row= $result->fetch_assoc()){
            ...
            '
            <tr>
            <td style="border: 2px solid black; padding: 10px;">' . $row["Student_ID"] . '</td>
            ...
            <td style="border: 2px solid black; padding: 10px;"><center><a href="student_details.php?Student_ID=' . $row['Student_ID'] . '" class="button inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 items-center">View</a></center></td>
            </tr>
            ';
        }
}
...
<script>
    function filterTable() {
        ...
        // Variable to check if any student is found
        ...
        for (var i = 1; i < rows.length; i++) {
            ...
            var course = rows[i].cells[2].textContent.toUpperCase();
            var classValue = rows[i].cells[3].textContent.toUpperCase();

            if (name.includes(filterName) && course.includes(filterCourse) && classValue.includes(filterClass)) {
        ...
        // Display an alert if no students are found
        if (!studentsFound) {
            alert("No students found. Resetting filters.");
            location.reload();
        }
    }
</script>
